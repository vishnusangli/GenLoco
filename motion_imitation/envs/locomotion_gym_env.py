...
class LocomotionGymEnv(gym.Env):
  ...
  def __init__(self,
    ...
    self._env_current_cumulative_rewards = 0
  ...
  def _unnormalize_action(self, action):
    # return action
    return self._action_offset + self._action_scale * action
  ...
  def step(self, action):
    ...
    self._robot.Step(unnormalized_action)
    ...
    self._env_current_cumulative_rewards += reward
    if done:
      self._robot.Terminate()
  ...
  @property
  def env_current_cumulative_rewards(self):
    return self._env_current_cumulative_rewards
