...
class HistoryStepsAPI:
    ...
    def begin_readings(self, max_length=900):
        self.history = np.zeros((max_length, len(self._joint_names)+len(self._joint_names)))
        self.curr_pointer=0
        self._max_length=max_length

    def take_reading(self, joint_obs, motor_torque):
        ...
        if self.curr_pointer >= self._max_length:
            ...
            self.history[self.curr_pointer, self.idx_num:] = motor_torque
            self.curr_pointer+= 1
    ...
    def next(self):
        ...
        self.curr_pointer+=1 
        return out[:self.idx_num], out[self.idx_num:]
