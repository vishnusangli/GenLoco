...
# import retarget_config_anymal_b as config
# import retarget_config_anymal_c as config
# import retarget_config_dkitty as config
# import retarget_config_sirius as config
# import retarget_config_mini_cheetah as config
...
def retarget_pose(robot, default_pose, ref_joint_pos, zero_rot=False):
...
def retarget_motion(robot, joint_pos_data, zero_rot=False):
  ...
  for f in range(num_frames):
    ...
    curr_pose = retarget_pose(robot, config.DEFAULT_JOINT_POSE, ref_joint_pos, zero_rot)
...
def output_motion(frames, out_filename):
  with open(out_filename, "w") as f:
    ...
    f.write("\"EnableCycleOffsetRotation\": false,\n")
    f.write("\n")
...
def main(argv):
  ...
  while True:
    ...
    for mocap_motion in mocap_motions:
      ...
      retarget_frames = retarget_motion(robot, joint_pos_data, zero_rot=True)
      output_motion(retarget_frames, f"a1_{mocap_motion[0]}.txt")
